...
function tanggal_indonesia($tanggal) {
    ...
    if(strpos($tanggal, ' s/d ') !== false) {
        $parts = explode(' s/d ', $tanggal);
        return format_tgl($parts[0]) . ' s/d ' . format_tgl($parts[1]);
    }
...
$resTotal = mysqli_query($koneksi, "SELECT COUNT(*) as total FROM pengeluaran WHERE level='transfer'");
$totalData = ($resTotal) ? mysqli_fetch_assoc($resTotal)['total'] : 0;

$sqlBase = "FROM pengeluaran WHERE level='transfer'";
if (!empty($search)) {
