...
$resTotal = mysqli_query($koneksi, "SELECT COUNT(*) as total FROM pengeluaran WHERE level='staff'");
$totalData = ($resTotal) ? mysqli_fetch_assoc($resTotal)['total'] : 0;

$sqlBase = "FROM pengeluaran WHERE level='staff'";
if (!empty($search)) {
    $sqlBase .= " AND (nama_staff_ruko LIKE '%$search%' OR tanggal LIKE '%$search%')";
}
