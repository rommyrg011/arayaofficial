...
if (isset($_GET['hapus_id'])) {
    ...
    header("Location: pengeluaranGaji");
    exit();
}
...
if (isset($_POST['simpanPengeluaran'])) {
    ...
    $cabang = $_POST['cabang'];
    $nama_staff_ruko = $_POST['nama_staff_ruko'];
    ...
    if (empty($id)) {
        mysqli_query($koneksi, "INSERT INTO pengeluaran (tanggal, cabang, nama_staff_ruko, pendapatan, potongan, total_bersih, level) VALUES ('$tanggal', '$cabang', '$nama_staff_ruko', '$pendapatan', '$potongan', '$total_bersih', '$level')");
        $_SESSION['notif'] = "Data Berhasil Disimpan";
    } else {
        mysqli_query($koneksi, "UPDATE pengeluaran SET tanggal='$tanggal', cabang='$cabang', nama_staff_ruko='$nama_staff_ruko', pendapatan='$pendapatan', potongan='$potongan', total_bersih='$total_bersih', level='$level' WHERE id_pengeluaran='$id'");
...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3 text-center">
            <h6 class="m-0 font-weight-bold" style="font-size:25px;">Data Pendapatan Staff</h6>
            <div class="mt-3">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" name="nama_staff_ruko" id="nama_staff_ruko" required>
                            <option value="" hidden>-- Pilih Staff --</option>
                            <?php
                            $query_staff = mysqli_query($koneksi, "SELECT nama_lengkap FROM user WHERE level='karyawan'");
                            while ($staff = mysqli_fetch_assoc($query_staff)) {
                                echo '<option value="' . $staff['nama_lengkap'] . '">' . $staff['nama_lengkap'] . '</option>';
                            }
                            ?>
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control rupiah-input" name="pendapatan" id="pendapatan" placeholder="Rp. 0" required>
                    </div>
                    <div class="form-group">
                        <label>Potongan</label>
                        <input type="text" class="form-control rupiah-input" name="potongan" id="potongan" placeholder="Rp. 0" value="Rp. 0" required>
                    </div>
                    <div class="form-group">
                        <label>Pendapatan Bersih</label>
                        <input type="text" class="form-control" name="total_bersih" id="total_bersih" placeholder="Rp. 0" readonly>
                    </div>
                </div>
...
$(document).ready(function() {
    ...
    $('#btnTambah').click(function() {
        $('#modalLabel').text('Tambah Data Pendapatan');
    ...
    $('#btnEdit').click(function() {
        if(selectedId && selectedData) {
            $('#modalLabel').text('Edit Data Pendapatan');
            $('#id_pengeluaran').val(selectedData.id_pengeluaran);
            $('#tanggal').val(selectedData.tanggal_raw);
            if(selectedData.cabang !== undefined) {
                $('#cabang').val(selectedData.cabang);
            } else {
                $('#cabang').val('');
            }
            $('#nama_staff_ruko').val(selectedData.nama_staff_ruko);
            $('#pendapatan').val(selectedData.pendapatan);
            $('#potongan').val(selectedData.potongan);
    ...
    $('#btnHapus').click(function() {
        if(selectedId) {
            if(confirm('Hapus data staff ' + selectedData.nama_staff_ruko + '?')) {
                window.location.href = 'pengeluaranGaji?hapus_id=' + selectedId;
            }
        }
    });
    ...
    function hitungPendapatanBersih() {
        var pendapatan = $('#pendapatan').val().replace(/[^0-9]/g, '');
        var potongan = $('#potongan').val().replace(/[^0-9]/g, '');
        
        pendapatan = parseInt(pendapatan) || 0;
        potongan = parseInt(potongan) || 0;
        
        var bersih = pendapatan - potongan;
        
        $('#total_bersih').val(formatRupiah(bersih.toString(), 'Rp. '));
    }
