...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3 text-center">
            <h6 class="m-0 font-weight-bold" style="font-size:25px;">Admin Tranfer</h6>
            <div class="mt-3">
        ...
        <form id="formPengeluaran" action="" method="POST" autocomplete="off">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <input type="hidden" name="level" id="level" value="transfer">
...
$(document).ready(function() {
    ...
    $('#btnEdit').click(function() {
        if(selectedId && selectedData) {
            ...
            var tgl_raw = selectedData.tanggal_raw;
            if(tgl_raw.includes(' s/d ')) {
                var tgl_split = tgl_raw.split(' s/d ');
                $('#tanggal_awal').val(tgl_split[0]);
                $('#tanggal_akhir').val(tgl_split[1]);
            } else {
                $('#tanggal_awal').val(tgl_raw);
                $('#tanggal_akhir').val(tgl_raw);
            }
    ...
    $('#btnHapus').click(function() {
        if(selectedId) {
            if(confirm('Hapus data pengeluaran ' + selectedData.nama_staff_ruko + '?')) {
                window.location.href = 'adminTransfer.php?hapus_id=' + selectedId;
            }
        }
    });
