...
$resTotal = mysqli_query($koneksi, "SELECT COUNT(*) as total FROM daftar_harga WHERE p_paket IN ('rental', 'tv')");
$totalData = ($resTotal) ? mysqli_fetch_assoc($resTotal)['total'] : 0;

$sqlBase = "FROM daftar_harga WHERE p_paket IN ('rental', 'tv')";
if (!empty($search)) {
...
// Order by p_paket digabungkan agar data mengelompok dengan rapi di tabel
$sqlData = "SELECT * " . $sqlBase . " ORDER BY p_paket ASC, id_harga ASC LIMIT $start, $length";
$resData = mysqli_query($koneksi, $sqlData);

$data = [];
$no = $start + 1;
$last_paket = ""; // Variabel pembantu untuk menyembunyikan nama paket yang duplikat berurutan
...
if ($resData) {
    while ($row = mysqli_fetch_assoc($resData)) {
        ...
        $data[] = [
            ...
            "p_paket"    => $display_paket, // Mengirim teks kosong jika nama paket sama dengan baris sebelumnya
            "durasi"     => $row['durasi'],
            "daf_harga"  => $format_harga,
            "raw_harga"  => $row['daf_harga']
        ];
        $last_paket = $row['p_paket']; // Update paket terakhir yang diproses
    }
}
